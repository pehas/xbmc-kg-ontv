...
def get_oc_cookie():
    ...
    try:
        ...
        try:
            ...
            try:    cookie['utmp'] = common.parseDOM(cookies, 'cookie', attrs={'name': '_utmp'},     ret='value')[0]
            except: cookie['utmp'] = common.parseDOM(cookies, 'cookie', attrs={'name': '__utmp'},    ret='value')[0]
            result['phpsessid'] = cookie['phpsessid']
            result['utmp']      = cookie['utmp']
        except: pass
...
def oc_category(id):
    ...
    if(item_list['sys_items']):
        for item in item_list['sys_items']:
            items.insert(0, {
                'label': item['label'],
                'path' : plugin.url_for('oc_go_to_page', id = id, page = item['page']) if( item['search'] == True ) else plugin.url_for('oc_category_pagination', id = id, page = item['page']),
                'icon' : item['icon']
            })
...
def get_movie_list(order_id, page = '0'):
    ...
    try:
        ...
        result = common.fetchPage({'link': BASE_API_URL, 'post_data':{'action[0]':'Video.getCatalog', 'offset[0]':str(offset), 'size[0]':str(size), 'order[0]':order_id}})
        kg_stats(BASE_URL, GA_CODE, NK_CODE)
...
def get_movie(id):
    ...
    is_playlist = False
    try:
        result = common.fetchPage({'link': BASE_API_URL, 'post_data':{'action[0]':'Video.getMovie','movie_id[0]':id}})
        kg_stats(BASE_URL, GA_CODE, NK_CODE)
